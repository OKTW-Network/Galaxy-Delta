execute unless entity @s[tag=placeFailed] run summon minecraft:armor_stand ~ ~ ~ {Silent:1b,CustomNameVisible:0b,NoGravity:1b,Invulnerable:1b,Small:1b,Marker:1b,Invisible:1b,Tags:["customBlock","haveGUI","galaxy.super_assembler","init"],HandItems:[{id:"minecraft:diamond_sword",Count:1b,tag:{CustomModelData:100}},{}]}
...
scoreboard players set @e[tag=customBlock,tag=galaxy.super_assembler,tag=init] sbldAttribute 1
scoreboard players remove @e[tag=customBlock,tag=galaxy.super_assembler,tag=init] sbldValue 0
execute as @e[tag=customBlock,tag=galaxy.super_assembler,tag=init] run function galaxy:recipe/super_assembler/get-option_default
execute as @e[tag=customBlock,tag=galaxy.super_assembler,tag=init] run function galaxy:gui/assembly_table/super/update
