...
execute if score #advanced_blast_furnace.checkFuel galaxy.recipe matches 1 store result score #processor.burnTimeMax galaxy.block store result score #processor.burnTime galaxy.block run data get storage galaxy:get item[{fuel:1b}].tag.CustomData.galaxy.tag.fuelABf 1
