...
execute store result score #1 calcu_temp run data get storage galaxy:get item[{input:2b}].Count 1
execute store result score #2 calcu_temp run data get storage galaxy:get recipe[0].input[1].Count 1
execute store result block ~ ~ ~ Items[{Slot:20b}].Count byte 1 run scoreboard players operation #1 calcu_temp -= #2 calcu_temp
