...
scoreboard players remove #block.elevator.find_target.recursiveStep galaxy 1
...
execute unless score #block.elevator.find_target.recursiveStep galaxy matches 1.. run tag @s add galaxy._STOP

execute if entity @s[tag=galaxy._STOP] run kill @s

execute if entity @s[tag=!galaxy._STOP] if score #block.elevator.find_target.recursiveStep galaxy matches 1.. run function galaxy:block/elevator/find_target/_recursive
