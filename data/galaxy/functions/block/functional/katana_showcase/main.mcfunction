...
execute unless data entity @s HandItems[0].Count if entity @s[tag=displaying] run function galaxy:weapon/katana/replace-hand_main/item_conversion-display
...
execute if entity @s[predicate=galaxy:hand_main-not_empty,predicate=!galaxy:armor_stand/hand_main-katana_display] run tag @s add throw
