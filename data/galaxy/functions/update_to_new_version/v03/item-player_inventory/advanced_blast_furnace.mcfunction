execute store result score #1 calcu_temp run clear @s minecraft:item_frame{advancedBlastFurnace:{}} 64
execute unless score #1 calcu_temp matches 1.. store result score #1 calcu_temp run clear @s minecraft:item_frame{EntityTag:{Tags:["block_placer","advanced_blast_furnace_placer"]}} 64
execute if score #1 calcu_temp matches 1.. run data modify storage galaxy:get item set value []
execute if score #1 calcu_temp matches 1.. run function galaxy:block/get/advanced_blast_furnace
...
execute if data entity @s Inventory[{tag:{advancedBlastFurnace:{}}}] run function galaxy:update_to_new_version/v03/item-player_inventory/advanced_blast_furnace
