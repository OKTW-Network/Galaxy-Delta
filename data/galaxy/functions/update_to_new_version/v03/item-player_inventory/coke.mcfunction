execute store result score #1 calcu_temp run clear @s minecraft:command_block{coke:{}} 64
execute if score #1 calcu_temp matches 1.. run data modify storage galaxy:get item set value []
execute if score #1 calcu_temp matches 1.. run function galaxy:material/get/coke
...
execute if data entity @s Inventory[{tag:{coke:{}}}] run function galaxy:update_to_new_version/v03/item-player_inventory/coke
