...
execute unless score #1 calcu_temp matches 1.. store result score #1 calcu_temp run clear @s minecraft:item_frame{EntityTag:{Tags:["entity_placer","katana_display_stand_placer","type-3"]}} 64
...
execute if data entity @s Inventory[{tag:{katanaShowcase:{type:3}}}] run function galaxy:update_to_new_version/v03/item-player_inventory/katana_display_stand-3
