...
execute as @e[tag=bulletDamage] run scoreboard players operation @s health -= @e[tag=galaxy.bullet,tag=flighting] bulletDamage
execute as @e[tag=bulletDamage,scores={health=..0}] at @s run function galaxy:damage/custom_death
execute as @e[tag=bulletDamage,scores={health=..0}] run scoreboard players add @e[tag=galaxy.bullet,tag=flighting,limit=1] sucBulletKill 1
execute as @e[tag=bulletDamage,scores={health=1..}] store result entity @s Health float 1 run scoreboard players get @s health
execute as @e[tag=bulletDamage,scores={health=1..}] run data modify entity @e[tag=galaxy.bullet,tag=flighting,limit=1] data.entityDamaged append from entity @s UUID
