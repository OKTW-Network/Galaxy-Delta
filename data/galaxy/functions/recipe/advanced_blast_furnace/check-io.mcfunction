...
execute if data storage galaxy:temp process[{output:1b,tag:{ingotSteel:{}}}] run function galaxy:recipe/advanced_blast_furnace/check-output_count
...
execute if data storage galaxy:temp process[{output:1b}].Count unless data storage galaxy:temp process[{output:1b,tag:{ingotSteel:{}}}] run scoreboard players set @s sucABfRcp -1
