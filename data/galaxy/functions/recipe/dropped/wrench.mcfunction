...
execute as @e[type=minecraft:item,tag=galaxy.recipe.dropped.wrench.material_1] at @s if score @s sucDropRcp matches 1 run playsound minecraft:entity.zombie_villager.converted block @a ~ ~ ~ 1 2
execute as @e[type=minecraft:item,tag=galaxy.recipe.dropped.wrench.material_1] at @s if score @s sucDropRcp matches 1 run function galaxy:tool/summon/wrench
execute as @e[type=minecraft:item,tag=galaxy.recipe.dropped.wrench.material_1] at @s if score @s sucDropRcp matches 1 run kill @e[type=minecraft:item,tag=galaxy.recipe.dropped.wrench.material_2,distance=..1,sort=nearest,limit=1]
execute as @e[type=minecraft:item,tag=galaxy.recipe.dropped.wrench.material_1] if score @s sucDropRcp matches 1 run kill @s
