...
execute as @e[tag=galaxy.dummy.droppedRecipeAnchor] at @s as @e[tag=galaxy._tag.TheseRecipeDroppedCandidate,nbt={Item:{id:"minecraft:stick",Count:1b}},dx=0,dy=0,dz=0,sort=nearest,limit=1] run tag @s add galaxy._tag.recipe.dropped.material.2
...
execute if entity @s[tag=galaxy._success.recipe.dropped] run function galaxy:recipe/dropped/wrench/fusion
