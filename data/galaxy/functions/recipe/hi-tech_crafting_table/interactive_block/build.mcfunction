...
function galaxy:recipe/hi-tech_crafting_table/interactive_block/get-item
...
data modify storage galaxy:recipe HTct.category[-1].icon.tag merge value {display:{Name:'{"translate":"gui.htct.interactive_block","italic":false}'},HideFlags:63,guiItem:1}
data remove storage galaxy:recipe HTct.category[-1].icon.tag.display.Lore
# data modify storage galaxy:recipe HTct.category[-1].icon set value {id:"minecraft:golden_hoe",Count:1b,tag:{display:{Name:'{"translate":"gui.htct.interactive_block","italic":false}'},HideFlags:63,CustomModelData:30103,guiItem:1}}
