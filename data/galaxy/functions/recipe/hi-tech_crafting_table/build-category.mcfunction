execute store result score #_build.HTct.item.remain galaxy.recipe store result storage galaxy:recipe HTct.category[-1].recipeTotal int 1 store result score #1 calcu_temp store result score #2 calcu_temp run data get storage galaxy:temp +recipe.build.input
...
execute store result score #1 calcu_temp run data get storage galaxy:recipe HTct.category[-1].recipeInFinalPage
