...
execute if score #_build.HTct.item.remain.inPage galaxy.recipe matches 0 run data modify storage galaxy:recipe hi-tech_crafting_table.category[-1].recipeList prepend from storage galaxy:temp +recipe.build.result
...
execute if score #_build.HTct.item.remain galaxy.recipe matches 1.. run function galaxy:recipe/hi-tech_crafting_table/build-recipe
