...
execute store result block ~ ~ ~ Items[{Slot:16b}].tag.gun.coolingDelay int 1 run scoreboard players get @s optCoolingDly
...
execute if data block ~ ~ ~ Items[{Slot:16b}].tag.gun.type run data modify block ~ ~ ~ Items[{Slot:16b}].tag.display.Name set value '{"translate":"item.galaxy.lasor","italic":false}'
