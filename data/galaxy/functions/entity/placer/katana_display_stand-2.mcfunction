...
execute unless entity @s[tag=placeFailed] unless entity @s[tag=placeCanceled] run summon minecraft:armor_stand ~ ~ ~ {CustomNameVisible:0b,Invulnerable:1b,Small:1b,Invisible:1b,Silent:1b,ShowArms:1b,DisabledSlots:4079166,Pose:{Head:[0f,0f,0f],Body:[0f,0f,0f],LeftArm:[0f,0f,0f],RightArm:[0f,0f,0f]},ArmorItems:[{},{},{},{id:"minecraft:wooden_sword",Count:1b,tag:{CustomModelData:20101}}],Tags:["customEntity","galaxy.katana_display_stand","type-2","noOverlap","init"],Team:"GalaxyCustomE"}

function galaxy:entity/directional-angle_8
scoreboard players set @e[tag=customEntity,tag=galaxy.katana_display_stand,tag=type-2,tag=init] kdsType 2
scoreboard players set @e[tag=customEntity,tag=galaxy.katana_display_stand,tag=type-2,tag=init] kdsStatus 1
tag @e[tag=customEntity,tag=galaxy.katana_display_stand,tag=type-2,tag=init] remove init

execute if entity @s[tag=placeFailed] as @a[tag=ThisPlaceEntity,gamemode=!creative] run function galaxy:entity/give/katana_display_stand-2
