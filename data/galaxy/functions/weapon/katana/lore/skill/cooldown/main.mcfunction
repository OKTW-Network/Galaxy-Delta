...
execute if score #weapon.katana.lore._buildSkillCooldown galaxy matches 1..2 run data modify storage galaxy:temp +weapon.katana.lore._finalValue set value '{"storage":"galaxy:temp","nbt":"+weapon.katana.lore.input.skill.cooldown","color":"dark_green"}'
execute if score #weapon.katana.lore._buildSkillCooldown galaxy matches 3 run function galaxy:weapon/katana/lore/skill/cooldown/modifier
