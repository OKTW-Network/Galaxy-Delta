...
execute if score #katana_act_flash Config matches 1 if entity @s[predicate=minecraft:sneaking,predicate=galaxy:weapon/posture-katana-1,scores={ktnSwapPullCd=0},tag=galaxy.ktnFirstAct] run tag @s add galaxy.reqKtnSwapPull
execute if score #katana_act_flash Config matches 1 if entity @s[predicate=minecraft:sneaking,predicate=minecraft:on_ground,predicate=galaxy:weapon/posture-katana-1,tag=galaxy.ktnFlashCharged,tag=!galaxy.ktnFirstAct] run tag @s add galaxy.reqKtnFlash
