...
scoreboard players remove #katana.action.flash._recursiveTimes galaxy 1
...
execute unless score #katana.action.flash._recursiveTimes galaxy matches 1.. run tag @s add galaxy._STOP

execute if entity @s[tag=galaxy._STOP] run kill @s

tag @s remove galaxy._success.weapon.katana.slash.pathPassed

execute if entity @s[tag=!galaxy._STOP] if score #katana.action.flash._recursiveTimes galaxy matches 1.. at @s run function galaxy:weapon/katana/action/flash/move/main
