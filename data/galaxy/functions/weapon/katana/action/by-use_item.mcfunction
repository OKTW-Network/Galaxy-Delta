...
execute if score #galaxy$katana_act_flash Config matches 1 if entity @s[predicate=minecraft:sneaking,predicate=galaxy:weapon/posture-katana-1,scores={galaxy.katana.swapPull.cooldown=0},tag=!cu._status.usingItemContinuously] run tag @s add galaxy._request.katana.swapPullReady
execute if score #galaxy$katana_act_flash Config matches 0 if entity @s[predicate=minecraft:sneaking,predicate=galaxy:weapon/posture-katana-1,scores={galaxy.katana.swapPull.cooldown=0},tag=galaxy._tag.katanaSwapPullReady] run tag @s add galaxy._request.katana.swapPull
...
execute if score #galaxy$katana_act_stun Config matches 1 if entity @s[predicate=!minecraft:sneaking,predicate=galaxy:weapon/posture-katana-2,scores={galaxy.katana.stun.cooldown=0}] run tag @s add galaxy._request.katana.stun
execute if score #galaxy$katana_act_flash Config matches 1 if entity @s[predicate=minecraft:sneaking,predicate=minecraft:on_ground,predicate=galaxy:weapon/posture-katana-1,scores={galaxy.katana.flash.cooldown=0},tag=cu._status.usingItemContinuously] run tag @s add galaxy._request.katana.flash
