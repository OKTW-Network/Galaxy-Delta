...
execute if score #galaxy$weapon.katana.action.flash Config matches 1 if entity @s[predicate=minecraft:sneaking,predicate=galaxy:weapon/posture-katana-1,tag=galaxy._tag.katanaSwapPullReady] run tag @s add galaxy._tag.requestKatanaActionSwapPull
