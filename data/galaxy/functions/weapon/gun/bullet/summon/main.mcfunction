...
scoreboard players set @e[tag=galaxy.bullet,tag=init,limit=1] bulletTraceCD 12
scoreboard players set @e[tag=galaxy.bullet,tag=init,limit=1] bulletDistance 0

function galaxy:weapon/gun/bullet/summon/offset/main

execute if score @e[tag=galaxy.bullet,tag=init,limit=1] bulletColor matches 17 run function galaxy:weapon/gun/bullet/summon/rainbow

tag @e[tag=galaxy.bullet,tag=init] remove init
