...
execute if score #1 calcu_temp matches 1.. as @e[tag=galaxy.dummy.bulletRotationAnchor,tag=init] at @s run function galaxy:weapon/gun/bullet/summon/offset/init/sequence/set-anchor/move-z_10
...
execute if score #1 calcu_temp matches 1.. as @e[tag=galaxy.dummy.bulletRotationAnchor,tag=init] at @s run function galaxy:weapon/gun/bullet/summon/offset/init/sequence/set-anchor/move-z_2
scoreboard players operation #2 calcu_temp %= #2 num
execute if score #2 calcu_temp matches 1 as @e[tag=galaxy.dummy.bulletRotationAnchor,tag=init] at @s run function galaxy:weapon/gun/bullet/summon/offset/init/sequence/set-anchor/move-z_1
