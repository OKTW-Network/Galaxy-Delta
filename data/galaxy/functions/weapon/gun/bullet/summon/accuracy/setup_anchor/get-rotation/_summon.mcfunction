...
data modify storage galaxy:temp +weapon.gun.bullet.summon.accuracy.rotation append from entity @e[tag=galaxy.dummy.bulletGetRotation,limit=1] Rotation
kill @e[tag=galaxy.dummy.bulletGetRotation]

execute if entity @e[tag=galaxy.dummy.bulletRotationAnchor] run function galaxy:weapon/gun/bullet/summon/accuracy/setup_anchor/get-rotation/_summon
